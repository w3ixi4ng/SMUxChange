...
function UpdateExistingMap({ map, setSelectedCourses, selectedCourses }: ChildProps) {
 ...
 const fetchSchoolCores = async (faculty: string, major: string, track: string) => {
   try {
     ...
     // Extract track electives if applicable
     if (track !== "") {
       let track_electives =
     ...
     for (let f of faculty) {
       ...
       if (mappable_mods[0].Majors["Second Major"]) {
         ...
         for (let second_major in second_major_obj) {
           records[second_major] =
             second_major_obj[second_major]["Major Elective"];
         }
       }
     }
 ...
 // === INITIALIZE EXISTING MAP COURSES ===
 useEffect(() => {
   if (map && map.map) {
     setSelectedCourses(map.map);


     // Count initially selected courses
     const initialCount = Object.values(map.map).reduce(
       (acc: number, curr: any) => {
         if (curr && curr.courses) return acc + curr.courses.length;
         return acc;
       },
       0
     );
     ...
     // Get areas that already have selected courses
     const areasWithCourses = Object.keys(map.map).filter(
       (area: string) =>
         map.map[area].courses && map.map[area].courses.length > 0
     );
 ...
 const handleSelectedCoursesChange = (
   ...
   setSelectedCourses(
     (prev: { [courseArea: string]: { limit: number; courses: string[] } }) => ({
       ...prev,
       [courseArea]: {
 ...
 return (
     <div
       className="w-[92%] max-w-[1300px] mx-auto mt-0 mb-12"
       style={{ color: "#102b72", transform: "translateX(-10px)" }}
     >
     {/* === 30% / 70% SPLIT LAYOUT === */}
     <div
        className={`${
          availableCourseAreasList.length <= 1
            ? "flex justify-center"
            : "grid lg:grid-cols-[32%_68%] gap-8 items-start"
        } w-full`}
      >
       {/* === LEFT PANEL (YOUR MAP) === */}
       <div className="bg-white/80 backdrop-blur-md border border-[#102b72]/20 rounded-2xl shadow-lg p-8">
         ...
         {selectedCount === 0 ? (
           ...
           // === SELECTED COURSES GROUPED BY AREA ===
       ...
       <div className="bg-white/80 backdrop-blur-md border border-[#102b72]/20 rounded-2xl shadow-lg p-8">
         ...
         {/* === EXISTING COURSE MAPS (EACH AREA) === */}
